...
import { RefreshCw, Upload, Download, Trash2, Check, Settings, Sliders, Layers } from 'lucide-react';
...
export default function SvgToPngTool({ triggerNotification, theme }: ToolProps) {
  ...
  const addFiles = (fileList: File[]) => {
    ...
    svgFiles.forEach((file, index) => {
      ...
      reader.onload = (e) => {
        ...
        // Parse SVG to find width, height and viewBox attributes
        let width = 512;
        let height = 512;
        try {
        ...
        setSvgs(prev => {
          ...
          if (updated.length === 1) {
            // Set first item's dimensions as default inputs
    ...
    triggerNotification(`Added ${svgFiles.length} SVG vector files.`);
  };
  ...
  const handleConvert = () => {
    ...
    // DPI Multiplier: standard screen is 96 DPI. Selected DPI scale is dpi / 96
    ...
    svgs.forEach((item) => {
      ...
      // Create a blob representing the SVG XML content to avoid URL encoding issues
      ...
      htmlImg.onload = () => {
        ...
        if (processedCount === svgs.length) {
          ...
          triggerNotification('All SVGs successfully converted to high-res PNG!');
        }
      };
  ...
  const downloadSingle = (item: SvgItem) => {
    ...
    link.download = item.name.replace(/\.svg$/i, '') + '.png';
  ...
  const downloadAllAsZip = async () => {
    ...
    for (let s of readySvgs) {
      ...
      const outputName = s.name.replace(/\.svg$/i, '') + '.png';
    ...
    const link = document.createElement('a');
    link.href = zipUrl;
    link.download = `rasterized_svg_images_${Date.now()}.zip`;
    link.click();
  ...
  return (
    <div id="svg-to-png-workspace" className={`p-6 rounded-2xl border ${theme === 'dark' ? 'bg-[#151924] border-slate-800' : 'bg-white border-slate-200'}`}>
      <div className="flex items-center gap-2.5 mb-2">
        ...
        <h2 className="text-xl font-bold tracking-tight">SVG to PNG Converter</h2>
      </div>
      <p className="text-xs text-slate-400 mb-6 font-medium">Convert SVG scalable vector graphics into crisp transparent PNG format with high-resolution export scales and custom DPI configurations.</p>
      ...
      {svgs.length === 0 ? (
        ...
        >
          ...
          <p className="text-[10px] text-slate-400 mt-1">Batch upload vector graphics. Supports customizable DPI grids and solid hex fallback colors offline.</p>
        </div>
      ) : (
                      ...
                      {item.status === 'success' && item.convertedUrl ? (
                        <div className="flex justify-between items-center mt-2">
                          <span className="text-[9px] font-bold text-emerald-500 flex items-center gap-1">
                            <Check className="w-3.5 h-3.5" /> PNG {formatSize(item.convertedSize)}
                          </span>
                          <button 
            ...
            <div className="lg:col-span-4 flex flex-col justify-between space-y-6">
              <div className={`p-4 rounded-xl border text-xs text-slate-400 space-y-5 leading-relaxed ${
                  ...
                  <div className="space-y-3">
                    <label className="flex items-center gap-2 cursor-pointer">
                      <input 
                        ...
                        className="rounded border-slate-300 text-emerald-600 focus:ring-emerald-500 accent-emerald-500"
                      />
                      <span className="font-bold text-slate-500">Transparent Background</span>
                    </label>

                    {!transparent && (
                      <div className="flex items-center gap-2.5">
                        <span className="font-bold text-slate-500 shrink-0">Backdrop Hex:</span>
                        <input 
                          type="color"
                          value={bgColor}
                ...
                <div className="border-t border-slate-200 dark:border-slate-800 pt-3 text-[10px] text-slate-400 space-y-1">
                  <p>• High-res 300 DPI multiplies dimensions by 3.125x for extreme print crispness.</p>
                </div>
              </div>
