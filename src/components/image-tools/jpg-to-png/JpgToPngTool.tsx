...
export default function JpgToPngTool({ triggerNotification, theme }: ToolProps) {
  ...
  const addFiles = (fileList: File[]) => {
    ...
    if (jpgs.length === 0) {
      triggerNotification('Please select valid JPG/JPEG files.');
      return;
    }
    ...
    triggerNotification(`Added ${jpgs.length} JPG files to queue.`);
  };
      ...
      htmlImg.onload = () => {
        ...
        if (ctx) {
          ...
          setImages(prev => prev.map(item => {
            if (item.id === img.id) {
              return { ...item, convertedUrl: pngDataUrl, status: 'success' };
            }
            return item;
          }));
        }
        ...
        if (processedCount === images.length) {
          ...
          triggerNotification('All JPG images successfully converted to lossless PNG format!');
        }
      };
      htmlImg.onerror = () => {
  ...
  const downloadAllAsZip = async () => {
    ...
    for (let img of readyImages) {
      ...
      const outputName = img.name.replace(/\.jpe?g$/i, '') + '.png';
  ...
  return (
    <div id="jpg-to-png-workspace" className={`p-6 rounded-2xl border ${theme === 'dark' ? 'bg-[#151924] border-slate-800' : 'bg-white border-slate-200'}`}>
      ...
      <p className="text-xs text-slate-400 mb-6 font-medium">Batch convert lossy JPG photographs into professional lossless PNG graphics with full resolution preservation.</p>
      ...
      {images.length === 0 ? (
        ...
        >
          ...
          <p className="text-[10px] text-slate-400 mt-1">Batch drag & drop. Processes fully in-browser without network transfers.</p>
        </div>
      ) : (
        <div className="space-y-6">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                      ...
                      {img.status === 'success' && img.convertedUrl ? (
                        <div className="flex justify-between items-center mt-2">
                          <span className="text-[9px] font-bold text-emerald-500 flex items-center gap-1">
                            <Check className="w-3.5 h-3.5" /> Transcoded
                          </span>
                          <button 
            ...
            <div className="lg:col-span-4 flex flex-col justify-between space-y-6">
              <div className={`p-4 rounded-xl border text-xs text-slate-400 space-y-3 leading-relaxed ${
                theme === 'dark' ? 'bg-[#0f111a]/45 border-slate-800' : 'bg-slate-50 border-slate-200'
              }`}>
                <p className="font-bold text-slate-700 dark:text-slate-300">Lossless Transcoding details:</p>
                <p>• Conversion reconstructs exact source RGB colors without any artifacts.</p>
                <p>• Retains the original dimension resolutions 100%.</p>
              </div>
                ...
                >
                  {isProcessing ? (
                    <>
                      ...
                      <span>Transcoding JPG to PNG...</span>
                    </>
                  ) : (
