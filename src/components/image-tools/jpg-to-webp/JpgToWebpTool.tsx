...
export default function JpgToWebpTool({ triggerNotification, theme }: ToolProps) {
  ...
  const addFiles = (fileList: File[]) => {
    ...
    if (jpgs.length === 0) {
      triggerNotification('Please select valid JPG/JPEG/PNG image files.');
      return;
    }
    ...
    triggerNotification(`Added ${jpgs.length} images to queue.`);
  };
    ...
    images.forEach((img) => {
      ...
      htmlImg.onload = () => {
        ...
        if (ctx) {
          ...
          // Render as WebP. In modern browsers, toDataURL('image/webp') is fully supported
        ...
        if (processedCount === images.length) {
          ...
          triggerNotification('All images successfully converted to WebP!');
        }
      };
      htmlImg.onerror = () => {
  ...
  const downloadSingle = (img: ImageItem) => {
    ...
    link.download = img.name.replace(/\.(jpe?g|png)$/i, '') + '.webp';
  ...
  const downloadAllAsZip = async () => {
    ...
    for (let img of readyImages) {
      ...
      const outputName = img.name.replace(/\.(jpe?g|png)$/i, '') + '.webp';
  ...
  return (
    <div id="jpg-to-webp-workspace" className={`p-6 rounded-2xl border ${theme === 'dark' ? 'bg-[#151924] border-slate-800' : 'bg-white border-slate-200'}`}>
      <div className="flex items-center gap-2.5 mb-2">
        <div className="p-2 rounded-lg bg-blue-50 text-blue-600 dark:bg-blue-950/50 dark:text-blue-400">
          <ImageIcon className="w-5 h-5" />
        </div>
        <h2 className="text-xl font-bold tracking-tight">JPG to WebP Converter</h2>
      </div>
      <p className="text-xs text-slate-400 mb-6 font-medium">Convert JPG/JPEG/PNG photographs into highly compressed next-gen WebP format with side-by-side split visual comparisons.</p>
      ...
      {images.length === 0 ? (
        ...
        >
          ...
          <p className="text-xs font-bold text-slate-500">Upload JPG, JPEG, or PNG files</p>
          <p className="text-[10px] text-slate-400 mt-1">Batch upload with split compare controls. All conversion runs offline inside browser sandboxes.</p>
        </div>
      ) : (
              ...
              <div className={`p-4 rounded-xl border text-xs text-slate-400 space-y-5 leading-relaxed ${
                ...
                <div>
                  ...
                  <label className="flex items-center gap-2 mb-4 cursor-pointer">
                    ...
                    />
                    <span className="font-bold text-slate-500">Lossless Encoding Mode</span>
                  </label>
                ...
                <div className="border-t border-slate-200 dark:border-slate-800 pt-3 text-[10px] text-slate-400 space-y-1">
                  <p>• WebP supports state-of-the-art predictive block encoding routines matching standard modern browser configurations.</p>
                </div>
              </div>
              ...
              <div className="space-y-2.5">
                ...
                >
                  {isProcessing ? (
                    ...
                    <span>Convert JPGs to WebP</span>
                  )}
                </button>
